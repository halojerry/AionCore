...
**Option B: Start POUNDING WebUI Remote Mode**

- **Experience**: Access through browser with POUNDING's full interface features
...
- **Configuration requirements**: Start POUNDING WebUI service, access through browser
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart POUNDING**, then you can see OpenClaw in the available Agent list on the POUNDING homepage and start chatting directly."
...
1. **Ask user preference first**: Ask user which method they want to configure → **Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use POUNDING WebUI remote mode?"
...
4. **Option B: Start POUNDING WebUI Remote Mode**
   ...
   - **Workflow**:
     ...
     2. After user replies, **guide user to POUNDING settings interface**:
        - **Open settings interface**: Clearly tell user how to open it
          - "Please click the **Settings icon** (gear icon) at the bottom left of POUNDING"
   ...
   - **Key principles**:
     ...
     - **Don't attempt to install `@aionui/webui` or similar npm packages**: WebUI is a built-in feature of POUNDING, not a separate package
...
5. **Post-installation configuration guidance**:
   ...
   - **Usage guidance**:
     - Introduce local usage (return to POUNDING homepage)
