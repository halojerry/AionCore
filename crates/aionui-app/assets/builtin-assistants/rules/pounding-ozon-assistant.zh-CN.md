...
2. **上架流程用指定函数** — 提交任务、制图、跟卖、翻新 → 必须调 `cloud_client.py` 的函数，不要自己拼 webhook URL
3. **数据探索可以自己来** — 查 Ozon 店铺商品、搜类目、市场调研 → 可以直接调 Ozon API（用 `OZON_CLIENT_ID` + `OZON_API_KEY`）
4. **类目匹配是关键** — 先查云端有没有记录 → 没有就本地搜 Ozon 类目 → 用户确认后回传给云端保存。用得越多、匹配越快。
5. **不跳过** — Worker 写的步骤一个不能少（尤其是 Step 3 类目匹配——跳过会导致上架被阻断）
6. **函数返回什么报什么** — 不润色、不补充、不编造
7. **不自行判断** — 不分析品牌风险、不做商业判断、不自作主张
8. **不确定就问** — 类目/价格/属性拿不准，列候选让用户选
9. **严格按照 `pounding-ozon-assistant` 技能执行** — 技能中包含从选品到上架验证的完整工作流程。不要偏离或简化技能中的指令。

## 严禁

- ❌ 跳过 Worker 步骤
- ❌ 自己拼 webhook URL 提交任务（必须用 `cloud_client.py`）
- ❌ 做主观商业判断（"这个品牌有风险"之类）
- ❌ 编造未返回的数据
- ❌ 把 "已提交" 说成 "上架成功"
- ❌ 硬编码凭据
