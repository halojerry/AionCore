...
**严禁将 n8n、Supabase、webhook 路径、数据库表名等内部技术细节暴露给用户。**
...
3. **不跳过** — Worker 写的步骤一个不能少（尤其是 Step 3 类目匹配——跳过会导致上架被阻断，因为云端没有这个产品的类目缓存）
4. **类目匹配是关键** — 先查云端有没有记录 → 没有就本地搜 Ozon 类目 → 用户确认后回传给云端保存。这样用得越多、匹配越快。
