...
mod tests {
    ...
    fn classify_error_detects_bundled_acp_validation_failure() {
        let error = ManagedAcpToolError::invalid(
            "bundled managed Codex ACP artifact failed validation under /app/resources/managed-resources/acp/codex-acp/0.16.0/linux-x64: managed ACP entrypoint missing",
        );
        let (kind, status_code) = classify_error(&error);
    ...
    fn bundled_validation_failure_does_not_fallback_to_remote_download() {
        ...
        let tool_root = runtime_root.join("codex-acp").join("0.16.0").join(spec.manifest_key);
